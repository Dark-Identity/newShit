...
app.get('/bet_data' , async (req , res)=>{
  ...
  console.log('what have i done wrong');
...
app.get('/get_payment' , auth , async (req,res)=>{
...
app.get('/get_all_members' , auth ,  async (req,res)=>{
...
app.get('/get_bet_history' , async (req,res)=>{
  ...
  if(temp_user['number']){
    ...
    let setteled_bets = await Bet.find({phone : temp_user['number'] , settled : true} , {_id : 0 , team_a : 1 , team_b : 1 , scoreDetails : 1 , final_score : 1 , date : 1 , profit : 1 , time : 1 , league : 1 , leagueId : 1});
    let unsetteled_bets = await Bet.find({phone : temp_user['number'] , settled : false} , {_id : 0 , team_a : 1 , team_b : 1 , scoreDetails : 1  , date : 1 , profit : 1 , time : 1 , league : 1 , leagueId : 1});
...
app.get('/settled_bets' , async (req,res)=>{
...
app.post('/delbet', async (req,res)=>{
   console.log(req.body.league_id);
   let bet = await Bet.findOne({leagueId : req.body.league_id});
   console.log(bet);

   if(bet){

     let res = check_date(bet['date'] , bet['time']);

     if(res){
       let id = parseInt(req.body.league_id);
...
app.post('/signup' , async (req, res)=>{
  ...
  let body = req.body;
  let parent = 0;
  // check for name duplicay in db if found
  // check for phone number duplicacy;
  let name_found = await User.findOne({user : body.name});
  ...
  if(name_found){
    ...
    let parent_found;
    const inv = await generate_inv_code();

    if(body.invitation_code !== 0){

      try {

         parent_found = await User.findOne({inv : body.invitation_code});
        ...
        if(
          parent_found
            ...
            increment_parent_mem(body.invitation_code , parent_found.members);
            res.send({user : 0});
        }else{
...
app.post('/login' , async (req , res)=>{
  ...
  if(
    ...
    //create a jwt tokens
    const token = db_user.generateToken();
    res.cookie('jwt' , token , {
...
async function increment_parent_mem(inv , prev_members){
  prev_members += 1;
  await User.findOneAndUpdate({inv : inv} ,{members : prev_members});
}
